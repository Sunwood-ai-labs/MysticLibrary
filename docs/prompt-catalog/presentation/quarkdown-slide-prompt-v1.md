...
description: Quarkdownを使用してプロフェッショナルなプレゼンテーションスライドを作成するためのプロンプト
category: writing
...
- Quarkdownを使用してプレゼンテーションスライドを作成する
- リリースノートや技術文書から魅力的なスライドを生成する
- アニメーション効果やレイアウト制御を活用する
...
1. リリースノートまたは技術文書の内容を用意する
2. プロンプトの `[ここにリリースノートや技術文書の内容を挿入]` 部分に内容を貼り付ける
3. Quarkdownでスライドを生成する

## 入力例

```text
プロジェクト: MysticLibrary v1.0
リリース日: 2026-02-23

主要機能:
- プロンプトカタログの自動生成
- 日本語と英語のバイリンガル対応
- VitePressベースのドキュメントサイト
```
...
- 色指定（`color:{#xxx}`）はエラーの原因になるため使用しない
- サイズ値は `tiny`, `small`, `normal`, `medium`, `large`, `huge` のみ使用
- 複雑なカスタム関数は避け、シンプルな構造にする
- `.fragment` を効果的に使用してプレゼンテーションに動きを付ける
