---
title: 断面ケーキジオラマ プロンプト
description: 断面カットされた層状ケーキがミニチュアのジオラマシーンに変換される、ハイパーリアリスティックなスチルライフ画像を生成するプロンプト集
category: creative
...
prompt_source: "prompts/docs-first/creative/visual/layered-cake-diorama-prompts_JP.md"
---
...
- 食品とミニチュアの融合を表現したアート作品の生成
- ハイエンドなフードフォトグラフィースタイルの画像作成
- クリエイティブなジオラマシーンのデザインインスピレーション
- 独創的な視覚的ストーリーテリングの参考

## プロンプト本文

このプロンプトは様々なジオラマテーマに適応可能です。以下に代表的な例を示します。
...
1. **テーマを選択**: 上記のパターンを参考に、作成したいジオラマのテーマを決定
2. **プロンプトをカスタマイズ**: テーマに合わせてケーキの層や要素を調整
3. **画像生成AIに入力**: Midjourney、Stable Diffusion、DALL-E などに入力
4. **生成結果を確認**: ハイパーリアリスティックなジオラマ画像を確認

## 入力例

```text
テーマ: 中世の村落
調整: ケーキの層に小さな家、井戸、市場を配置
```

## 出力例

生成される画像には以下の特徴が含まれます：

- 断面カットされた層状ケーキの構造
- 各層に展開される詳細なミニチュアシーン
- リッチなテクスチャ（スポンジ、ナッツ、種、土風のフィリング）
- 根やミニチュア水路による自然な統合
- スタジオライティングによるドラマチックな影
- ハイエンドフードフォトグラフィーと精巧なミニチュアの融合

## 注意点

- 超高精細度の画像生成が必要です
- スタジオライティングとドラマチックな影を強調します
- クリアなフォーカスと鋭いエッジを目指します
- フードフォトグラフィーのクオリティを維持します
- ミニチュアのリアリズムを重視します
