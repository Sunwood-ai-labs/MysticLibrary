---
title: Minecraft MCP前提プロンプト
description: Minecraft MCPを使用して自律的に建築を行うBotのための前提プロンプト
...
- MinecraftでMCPを使用した自律型建築Botを作成したい場合
- マインクラフト内での自動建築システムを構築したい場合
- MCP（Model Control Protocol）の実装テストを行いたい場合
...
- このプロンプトはMCP（Model Control Protocol）対応のツールが必要です
- クリエイティブモードでの動作を前提としています
- Bot自身の位置管理が最優先事項となります
