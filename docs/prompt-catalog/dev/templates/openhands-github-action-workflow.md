---
title: OpenHands GitHub Action ワークフロー
description: GitHub/Firejo で OpenHands エージェントを自動実行し、Issue/PR を解決してドラフト PR を作成するワークフロー
category: dev
...
prompt_source: "prompts/docs-first/dev/templates/openhands-github-action-workflow_JP.md"
---

# OpenHands GitHub Action ワークフロー

## 想定用途

- Issue にラベルまたはメンションで OpenHands エージェントを自動起動
- バグ修正や機能実装を AI エージェントに自動化
- ドラフト PR の自動作成
...
1. リポジトリの `.github/workflows/` ディレクトリに `openhands-resolver.yml` として保存
2. GitHub リポジトリ設定で `ANTHROPIC_API_KEY` シークレットを追加
3. 必要に応じて `LLM_MODEL` 変数を設定
4. Issue に `fix-me` ラベルを付けるか、コメントに `@openhands-agent` を含める

## 入力例

```text
Issue #123 に「fix-me」ラベルを付与
または
Issue コメント: "@openhands-agent ログイン機能のバグを修正してください"
```

## 出力例

```text
🎉 OpenHands完了！ドラフトPR #124 を作成しました。
```

## 注意点

- Docker コンテナを使用するため、GitHub Enterprise Server では追加設定が必要
- 実験版を使用するには `fix-me-experimental` ラベルまたは `@openhands-agent-exp` メンションを使用
- OpenHands は変更を自動でコミットし、ドラフト PR を作成
