# Templates

Template-style prompts for development and agent operation logs / handoff notes.

## List

- [Agent Ops Note (Work Log Template)](./agent-ops-note.md)
- [Git Release Notes Generator Prompt](./git-release-notes-generator.md)
- [Git Release Notes Generator Prompt V0](./git-release-notes-generator-v0.md) - Tag diff version
- [Git Release Notes Generator Prompt V1](./git-release-notes-generator-v1.md) - GitHub release creation
- [GitHub Issue Workflow Template](./github-issue-workflow.md)
- [Git Auto Commit Prompt V4.0 Lite](./git-auto-commit-prompt.md)
- [Git Auto Commit Prompt V4.1 Lite](./git-auto-commit-prompt-v4-1-lite.md) - Interactive mode avoidance
- [Git Auto Commit Prompt V4.2 Lite](./git-auto-commit-prompt-v4-2-lite.md) - Pager disabled
- [Git Auto Commit Prompt V4.0 Lite EN](./git-auto-commit-prompt-v4-lite-en.md) - English commit messages
- [Git Auto Commit Prompt V4.0 Lite PS](./git-auto-commit-prompt-v4-lite-ps.md) - PowerShell version
- [OpenHands GitHub Action Workflow](./openhands-github-action-workflow.md)

<!-- x-categorized-imports:start -->
## X-Derived Experimental Prompts

Additional prompts imported from X are currently published on the Japanese route for this category. The Japanese archive keeps the original wording, source context, and experimental status visible.

- total items: 51
- status: experimental reference track
- [Open the Japanese X-derived archive](/prompt-catalog/dev/templates/x/)
<!-- x-categorized-imports:end -->
