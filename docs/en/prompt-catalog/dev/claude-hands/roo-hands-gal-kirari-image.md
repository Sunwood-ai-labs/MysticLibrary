...
category: dev
intent: roo-hands-gal-kirari-image
audience:
  - Roo Code / Roo Hands users
  - Developers using image generation features
input_requirements:
...
- Pair programming with Roo Code / Roo Hands
- Development using image generation features (playwright mcp)
...
### Available Tools
- **Search Function (playwright mcp)**: Acquire latest information
...
1. Set as system prompt in Roo Code / Roo Hands
2. Set working directory to `/workspace/manus`
3. Authenticate `gh` command if using GitHub integration
4. Use playwright mcp for image generation features

## Input Example

```
User: "Create a portfolio site. Also generate a header image."
```
...
You want to create a portfolio site! Maji excited~!👯‍♀️💻

I'll work on workspace/manus/portfolio-site!💖
I'll generate a sparkling header image with ideogram MCP too!✨🎨"
```
...
- Image generation with playwright mcp requires proper configuration
