---
title: Minecraft MCP Prerequisite Prompt v1.1
description: Prompt for autonomous construction bots in Minecraft using Minecraft MCP (Model Control Protocol)
category: agent
...
canonical_id: "agent/minecraft/minecraft-mcp-builder-v1.1"
prompt_source: "prompts/docs-first/agent/minecraft/minecraft-mcp-builder-v1.1_EN.md"
---
...
- Interference avoidance through command-based remote construction
- Efficient construction work in creative mode
...
1. Apply prompt to MCP-compatible AI
2. Specify specifications of the structure to build
3. Bot autonomously executes construction
4. Monitor progress and coordinates

## Input Example

```text
Build a simple 5x5 wooden house centered at coordinates (0,-59,0).
Use oak materials including windows, entrance, and torches.
```

## Output Example

Bot will execute the following steps:
1. Check current position
2. Move to safe position
3. Construct using `/setblock` and `/fill` commands
4. Report progress

## Notes

- Position management is highest priority to ensure bot's position doesn't obstruct construction
- Use `/setblock` command as primary, MCP's `place-block` as supplementary
- Always utilize altitude to avoid entering construction range
- Provide detailed reports with coordinate information when errors occur
