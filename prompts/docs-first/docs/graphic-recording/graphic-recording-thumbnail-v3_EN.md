---
title: "Graphic Recording Thumbnail V3 Ryoga (Simple)"
description: "Prompt for generating blog article thumbnails using Ryoga color palette without editing features"
canonical_id: "docs/graphic-recording/graphic-recording-thumbnail-v3"
canonical_doc: "docs/en/prompt-catalog/docs/graphic-recording/graphic-recording-thumbnail-v3.md"
locale: EN
docs_first: true
last_synced: 2026-03-18
---

<!-- Generated from docs/en/prompt-catalog/docs/graphic-recording/graphic-recording-thumbnail-v3.md. Edit docs/ instead. -->

# Graphic Recording Thumbnail Prompt v3 ~Ryoga~

This prompt generates **high-quality blog article thumbnails** that sublimate the aesthetics of summer coolness and Japanese harmony into modern times. Create visually polished 16:9 aspect ratio infographic thumbnails that convey content at a glance, like top-tier consultants create, using ***Font Awesome extensively*** with Ryoga's summer aesthetic.

## Basic Structure

### Prompt Format for Thumbnail Creation
