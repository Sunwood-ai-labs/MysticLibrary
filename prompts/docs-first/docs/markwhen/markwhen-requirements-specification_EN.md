---
title: "Markwhen Requirements Specification"
description: "Requirements specification defining the syntax and features of markwhen, a text-based timeline creation tool"
canonical_id: "docs/markwhen/markwhen-requirements-specification"
canonical_doc: "docs/en/prompt-catalog/docs/markwhen/markwhen-requirements-specification.md"
locale: EN
docs_first: true
last_synced: 2026-03-19
---

<!-- Generated from docs/en/prompt-catalog/docs/markwhen/markwhen-requirements-specification.md. Edit docs/ instead. -->

---
title: My Timeline
---

2023-01-01: New Year's Day

2023-02-14: Valentine's Day

_-_-_break_-_-_

---
title: Another Timeline
---

2023-07-04: Independence Day
